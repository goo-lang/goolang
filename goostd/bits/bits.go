...
// rev8tab is vendored verbatim from Go 1.26 math/bits (src/math/bits/bits_tab.go),
// inlined here (see len8tab). rev8tab[i] is the byte i with its 8 bits reversed.
const rev8tab = "" +
...
// overflowError and divideError are the panic values for Div. Upstream declares
// them as error values of an unexported errorString type (see
// bits_errors_bootstrap.go). Goo lacks named string types with methods and
// error-interface boxing, so they are string consts here — panic(overflowError)
// prints the identical "panic: runtime error: ..." message and aborts. This is
// a localized, documented deviation from the verbatim-vendoring rule; the Div
// function bodies below are verbatim.
const overflowError = "runtime error: integer overflow"
const divideError = "runtime error: integer divide by zero"

// NOTE: upstream orders Div, Div32, Div64. Div is moved AFTER Div32/Div64 here
// because Goo's codegen prototype pre-pass does not forward-declare aggregate-
// (multi-value)-return functions, so Div calling Div32/Div64 before their
// definitions fails. This is a file-layout change only; the bodies are verbatim.
