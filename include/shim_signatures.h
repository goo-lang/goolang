...
// A call's return type. Most shims return a plain builtin/slice; three
// need a builder beyond a bare TypeKind (error, []string, and
// strconv.Atoi's !int error-union) — SHIM_RET_ATOI_RESULT is a dedicated
// tag for that one irregular case rather than a general "builder function
// pointer" for a table with exactly one such entry.
typedef enum {
    SHIM_RET_VOID,
    SHIM_RET_STRING,
    SHIM_RET_FLOAT64,
