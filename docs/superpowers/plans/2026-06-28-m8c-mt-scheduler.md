...
- **No header edit.** Leave the now-unused `current_goroutine`/`main_context` fields in `struct goo_scheduler` (`include/runtime.h:384-391`) in place to avoid a `make clean`. The `_Thread_local` worker state supersedes them; they are simply no longer read or written. (Spec §4.1, §5)
