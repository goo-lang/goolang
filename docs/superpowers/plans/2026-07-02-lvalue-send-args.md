...
- Select-case send (separate copy of the same flaw): `src/codegen/statement_codegen.c:1813-1827` — `if (value_val->is_lvalue) data_ptr = value_val->llvm_value; else { alloca of LLVMTypeOf + store }`. NOTE: this copy's rvalue branch also lacks elem-type coercion (allocas `LLVMTypeOf(value)`), a narrower pre-existing gap — Task 2 fixes the LVALUE path at this site with a load (so lvalues at least go through a correct-width load of their own type and land in the rvalue branch); upgrading the select rvalue branch to full elem-type coercion is explicitly OUT OF SCOPE (report it if the probe trips it, but the probe sends an int32 field into a chan int64 via select, which after the load hits the same-shape alloca — see Task 2 Step 1 note).
...
- Task 2 deliberately does NOT upgrade the select-send rvalue branch's missing elem-type coercion (pre-existing, narrower gap) — but its probe WILL detect it if the lvalue load exposes it; the STOP-and-report note covers that fork.
